...
const uint8_t bakePhaseStrPosition[] =
{190, 202, 198, 128, 128, 0};
...
void bake()
{
	...
	while (1)
	{
		...
		{	
			...
			Serial.println("Bake aborted because of thermocouple error!");
		...
		switch (bakePhase)
		{
			case BAKING_PHASE_HEATUP:
				...
				if (prefs.bakeTemperature - currentTemperature < 15.0)
				{
					...
					Serial.println("Move to bake phase");
				}
				break;
...
void displayBakePhase(uint8_t phase, boolean abortDialogIsOnScreen)
{
	...
	Serial.println("Baking phase = " + String(bakePhaseStr[phase]));
}
...
//	Serial.println("Duration=" + String(duration) + "	minutes=" + String(minutes));
	return minutes * 60;
}
